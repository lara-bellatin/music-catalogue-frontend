  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      if (selectedEntities.length > 0) {
        selectedEntities.forEach(entityType => params.append("entity_type", entityType));
      }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-12">
        <header className="text-center">
          <h1 className="text-3xl font-semibold tracking-tight">Music Catalogue</h1>
        </header>

        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-slate-600" htmlFor="search-input">
              Search
            </label>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-sm font-medium text-slate-600">Included Entities</span>
            <ToggleGroup.Root
              type="multiple"
                  ...
                  <article className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">ID: {result.entity_id}</p>
                      </div>
                    ...
                    <p className="mt-2 text-sm text-slate-500">Rank score: {result.rank.toFixed(2)}</p>
                  </article>
                </li>
              ))}
            </ul>
